...
/// Converts the termina [event](termina::Event) to a terminput [event](Event).
pub fn to_terminput(value: termina::Event) -> Result<Event, UnsupportedEvent> {
    Ok(match value {
        ...
        termina::Event::Key(key_event) => Event::Key(key_event_to_terminput(key_event)?),
        termina::Event::Mouse(mouse_event) => Event::Mouse(mouse_to_terminput(mouse_event)),
        termina::Event::Paste(value) => Event::Paste(value),
        termina::Event::WindowResized { rows, cols } => Event::Resize {
            cols: cols as u32,
            rows: rows as u32,
        },
...
/// Converts the terminput [event](Event) to a termina [event](termina::Event).
pub fn to_termina(value: Event) -> Result<termina::Event, UnsupportedEvent> {
    Ok(match value {
        ...
        Event::Key(key_event) => termina::Event::Key(key_event_to_termina(key_event)),
        Event::Mouse(mouse_event) => termina::Event::Mouse(mouse_to_termina(mouse_event)?),
        Event::Paste(value) => termina::Event::Paste(value),
        Event::Resize { cols, rows } => termina::Event::WindowResized {
            cols: cols
                .try_into()
...
fn mouse_to_terminput(value: termina::event::MouseEvent) -> MouseEvent {
    MouseEvent {
...
fn mouse_to_termina(value: MouseEvent) -> Result<termina::event::MouseEvent, UnsupportedEvent> {
...
fn key_event_to_terminput(value: termina::event::KeyEvent) -> Result<KeyEvent, UnsupportedEvent> {
    Ok(KeyEvent {
...
fn key_event_to_termina(value: KeyEvent) -> termina::event::KeyEvent {
...
fn key_state_to_termina(value: KeyEventState) -> termina::event::KeyEventState {
    termina::event::KeyEventState::from_bits_retain(value.bits())
}
